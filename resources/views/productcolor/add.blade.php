...
<div class="box box-primary">
  <div class="box-header with-border">
    <h3 class="box-title">添加产品颜色分布</h3>
  </div>
  <br>
  <form class="form-horizontal" id="FormView" action="/productcolor/save" method="post">
	...
	<div class="form-group">
		...
		<div class="col-sm-10" id="countries">
		     ...
		     <label style="width:150px;height:35px;border:1px solid silver;margin:1px;float:left;">
                 <input class="mycountries" type="checkbox" name="countries[]" value="{{$av->CountryID}},{{$av->EnglishShort}}" @foreach( $countryAreaMapping as $mk => $mv ) @if(in_array($av->CountryID,$mv)) area_{{$mk}}="true" @endif @endforeach style="width:30px;" >
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
		    ...
		    @foreach( $product_models as $pk=>$pv )
				<option value="{{$pv->PID}},{{$pv->ProductCode}}">{{$pv->ProductCode}}</option>
		    @endforeach
			</select>
		</div>
	</div>
