$(function(){
	...
	$(".right-box a").click(function(){
		$(".right-box li").removeClass("active");
		$(this).parents("li").addClass("active");
		var index=$(this).parents("li").index();
		$(".left-box>div").hide();
		$(".left-box>div").eq(index).show();
	})
	//常见问题效果
	...
	//常见问题关键词过滤：给予jcfilter.js
	$("#filterFAQ").jcOnPageFilter({
...
//		textColorForHighlights:'#000000',
		caseSensitive:false,
		hideNegatives:true,
		parentLookupClass:'question-item',
		childBlockClass:'questionF',
		boxClass:"question-box",
	});
	$(".question a").click(function(){
		var list_l=$(".question-list").length;
		var list_d=0;
		for (var i = 0; i <list_l; i++) {
			var this_child_n=$(".question-list").eq(i).find(".question-item").length;
			// var this_child_d=$(".question-list").eq(i).find(".question-item[style='display:none']").length;
			var this_child_d=0;
			for(var j=0;j<this_child_n-1;j++){
				var this_style=$(".question-list").eq(i).find(".question-item").eq(j).attr("style");
				if(this_style=="display: none;"){
					this_child_d++
				}
			}
			if ((this_child_d+1)==this_child_n) {
				$(".question-list").eq(i).hide();
				list_d++;
			}else{
				$(".question-list").eq(i).show();
			};
		};
		if(list_d==list_l){
			$(".search-rez").fadeIn();
